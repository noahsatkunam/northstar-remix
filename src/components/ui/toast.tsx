...
const ToastProgress = React.forwardRef<HTMLDivElement, ToastProgressProps>(
  ({ className, duration = 5000, style, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "absolute bottom-0 left-0 h-1 w-full bg-primary/20 group-[.destructive]:bg-destructive-foreground/20",
        className,
      )}
      {...props}
    >
      <div
        className="h-full bg-primary group-[.destructive]:bg-destructive-foreground"
        style={{
